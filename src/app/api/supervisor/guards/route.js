// src/app/api/supervisor/guards/route.js - UPDATED to allow management access
...
// GET all guards with their shift status
export async function GET() {
  try {
    ...
    // Check if user is supervisor OR management (UPDATED)
  ...
  try {
    ...
    if (action === 'start_shift') {
      ...
      const newShift = {
        ...
        checkOutTime: null,
        status: 'active',
        location: `Started by ${session.user.role === 'management' ? 'Management' : 'Supervisor'}`,
        notes: notes || `Shift started by ${session.user.role}: ${session.user.name}`,
        supervisorStarted: session.user.role === 'security_supervisor',
        managementStarted: session.user.role === 'management', // NEW
        supervisorId: session.user.role === 'security_supervisor' ? session.user.id : null,
        managementId: session.user.role === 'management' ? session.user.id : null, // NEW
        supervisorName: session.user.role === 'security_supervisor' ? session.user.name : null,
        managementName: session.user.role === 'management' ? session.user.name : null, // NEW
        createdAt: new Date()
      }
      ...
      const result = await checkins.updateOne(
        { _id: activeShift._id },
        {
          $set: {
            ...
            status: 'completed',
            shiftDuration,
            notes: notes || `Shift ended by ${session.user.role}: ${session.user.name}`,
            supervisorEnded: session.user.role === 'security_supervisor',
            managementEnded: session.user.role === 'management', // NEW
            supervisorEndId: session.user.role === 'security_supervisor' ? session.user.id : null,
            managementEndId: session.user.role === 'management' ? session.user.id : null, // NEW
            supervisorEndName: session.user.role === 'security_supervisor' ? session.user.name : null,
            managementEndName: session.user.role === 'management' ? session.user.name : null, // NEW
            updatedAt: new Date()
          }
        }
      )
